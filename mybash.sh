...
if [[ ! $CMD_FILE == "" ]]
    ...
    if [[ $CMD_FILE == *"rm"* || $CMD_FILE == *"mv"* ]]
        ...
        if [[ $CMD_FILE == *"mv"* ]]
            then
            $(echo $CMD_FILE | cut -d\   -f2)
            logger -p local6.debug -- SESSIONHISTORY=$$, USER=$USER, OS=$distro, EVENTSOURCEIP=$EVENTSOURCE_IP, PWD=$DIR, CMD="${CMD}", IP=${IP}, EventType="FS", Eve    nt_Action="WRITE", FileDir=$(echo "${CMD}" | awk -F'[ \t]+' '{print $NF}' | rev | sed -E 's/^[[:space:]]+//')
        fi
    fi
